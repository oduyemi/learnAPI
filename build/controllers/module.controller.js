...
exports.deleteModule = exports.toggleModulePublish = exports.updateModule = exports.getModule = exports.getModules = exports.getModulesByCohort = exports.createModule = void 0;
...
const createModule = async (req, res) => {
    try {
        ...
        const { cohort, title, desc, weekNumber, session, video, text, resources, quizzes, assignment, order, releaseDate, isPublished, } = req.body;
        ...
        if (!cohort ||
            ...
            return res.status(400).json({
                success: false,
                message: "Cohort, title, description, week number, session, order and release date are required.",
            });
        }
        if (!mongoose_1.default.Types.ObjectId.isValid(cohort)) {
            return res.status(400).json({
                success: false,
        ...
        if (!Number.isInteger(parsedWeekNumber) ||
            parsedWeekNumber < 1) {
            return res.status(400).json({
                success: false,
        ...
        if (!video &&
            ...
            return res.status(400).json({
                success: false,
                message: "A module must contain video, text or at least one resource.",
            });
        }
        const existingModule = await module_model_1.default.findOne({
            cohort,
        ...
        if (existingModule) {
            return res.status(409).json({
                success: false,
                message: `Session ${session.toUpperCase()} for week ${parsedWeekNumber} already exists for this cohort.`,
            });
        }
        const module = await module_model_1.default.create({
            cohort,
            title: cleanedTitle,
            desc: cleanedDesc,
            weekNumber: parsedWeekNumber,
        ...
        if (error?.code === 11000) {
            return res.status(409).json({
                success: false,
                message: "A module already exists for this cohort, week and session.",
            });
        }
...
const getModulesByCohort = async (req, res) => {
    try {
        await (0, db_1.default)();
        const cohortId = getParamId(req.params.cohortId);
        if (!cohortId || !mongoose_1.default.Types.ObjectId.isValid(cohortId)) {
            return res.status(400).json({
                success: false,
                message: "Invalid cohort ID.",
            });
        }
        const cohort = await cohort_model_1.default.findById(cohortId).select("title code status courses");
        if (!cohort) {
            return res.status(404).json({
                success: false,
                message: "Cohort not found.",
        ...
        const modules = await module_model_1.default.find(filter)
            .populate({
            path: "quizzes",
        ...
        console.error("Get Modules By Cohort Error:", error);
        return res.status(500).json({
            success: false,
...
exports.getModulesByCohort = getModulesByCohort;
// ADMIN
...
const updateModule = async (req, res) => {
    try {
        ...
        const { title, desc, weekNumber, session, video, text, resources, quizzes, assignment, order, releaseDate, isPublished, } = req.body;
        if (title !== undefined) {
            const cleanedTitle = title.trim().replace(/\s+/g, " ");
            if (!cleanedTitle) {
                return res.status(400).json({
        ...
        const duplicate = await module_model_1.default.findOne({
            _id: { $ne: module._id },
        ...
        if (duplicate) {
            return res.status(409).json({
                success: false,
                message: "Another module already exists for this cohort, week and session.",
            });
        }
        await module.save();
        await module.populate([
...
exports.toggleModulePublish = toggleModulePublish;
